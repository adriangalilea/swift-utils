...
**Artwork first; a chip is one mark standing frameless.** The artwork IS the chip: badge artwork (HDR10, HDR10+, the tabler 4K / 8K / HD / SD badges) keeps its own box, a lockup keeps its shape, a symbol stands alone - no pill, no disc, no kind glyph around a mark. A DRAWN word-badge (rounded box + semibold word, the HDR10 badge's geometry: box ≈ 1.5 × cap height, radius 0.25 × box, stroke 0.08 × box) exists only for a value with no artwork anywhere: 720p, HLG, channels, Remux, WEB-DL / WEBRip / HDTV / CAM, DTS:X (its only vector is CC BY-NC-SA), TrueHD when the Atmos lockup already says Dolby, AAC / PCM / ALAC / MP3 / MP2 / Vorbis, castellano / latino / a code, every cut but IMAX. Per axis: picture = resolution badge + range mark (SDR unsaid); sound = object mark, then the codec (lockup, or a word when an object lockup leads), then the channels; tier = disc lockup (Ultra HD Blu-ray when the strip knows 2160p) + Remux; lang = flag + castellano at the rail; cut = IMAX or a word. Two rungs, one threshold (`SpecChip.rail` = 32): lockups at and above it, brand symbols (Dolby D, dts, the Blu-ray glyph, the flag) and the small badges below. A strip packs an axis at `.inkTight` and separates axes at `.inkGap`; a delta trails the axis's last mark.
