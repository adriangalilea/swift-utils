...
**The marks are real.** `Mark` (generated by `brandgen` into `Resources/Marks.xcassets`, provenance per file in `Resources/PROVENANCE.md`) holds the Dolby double-D and the Dolby Vision / Atmos / TrueHD / Digital Plus / Digital lockups, the dts symbol and wordmark, DTS-HD Master Audio, the HDR10 and HDR10+ badges, ULTRA HD, FLAC, Opus, Blu-ray Disc (lockup and glyph), Ultra HD Blu-ray, DVD, IMAX and the Spanish civil flag - simple-icons (CC0) where it has them, Wikimedia Commons public-domain files everywhere else, all normalised to one ink at review, the flag alone keeping its colours. Trademarks, used nominatively: a chip says a copy carries the format the mark names. `Marks.swift` is the one binding from a value to its artwork; a value without an entry renders as words, exactly as before.

**Two rungs, one threshold** (`SpecChip.rail` = 32). At and above it the LOCKUP replaces the word it stands for and the rest stays text: "4K" + the Dolby Vision logotype, the Dolby Atmos logotype + "TrueHD 7.1", the Blu-ray logotype + "Remux" (Ultra HD Blu-ray when the strip knows the copy is 2160p), the flag + "castellano", IMAX alone. Below it the SYMBOL stands in: the double-D in the disc + "DV" or "Atmos", the dts mark + "X", the HDR10+ badge alone, "4K" as text. A disc-shaped symbol (aspect ≤ 1.6) takes the pill's cap; a wide one runs inline. DTS:X has no artwork anywhere: it is the dts wordmark + ":X" set in type. Sizes: a lockup stands 0.50 × height, a disc symbol 0.6 × height across.

**Tone is the consumer's call**: `tone: .ink` (default) paints a mark in the label's colour, one ink on one ground; `.brand` paints it its official hex - on the unfilled weights only (a filled chip keeps one ink), and never for a brand whose official colour is near-black (Dolby, HDR10, DVD: black on a dark chip is a missing logo, not a brand statement). The flag ignores tone by construction.
