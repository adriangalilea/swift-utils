...
**Artwork first; a chip is one mark standing frameless.** The artwork IS the chip: a brand lockup keeps its shape, a brand symbol stands alone - no pill, no disc, no kind glyph around a mark. EVERY BOXED VALUE is one DRAWN badge in one geometry (box height = chip height, radius 0.25, stroke 0.08, side pad 0.26, word 0.62 semibold - the HDR10 badge's own proportion, box ≈ 1.5 × cap): SD / HD / 4K / 8K / 720p, HDR10 / HDR10+ / HLG, channels, Remux, WEB-DL / WEBRip / HDTV / CAM, DTS:X (its only vector is CC BY-NC-SA), TrueHD when the Atmos lockup already says Dolby, AAC / PCM / ALAC / MP3 / MP2 / Vorbis, castellano / latino / a code, every cut but IMAX. Badge artwork from three sources (tabler, Commons) put three stroke weights in one row and the eye read three families, so it stays in the catalog as reference and binds nothing. Per axis: picture = resolution badge + range mark (SDR unsaid); sound = object mark, then the codec (lockup, or a word when an object lockup leads), then the channels; tier = disc lockup (Ultra HD Blu-ray when the strip knows 2160p) + Remux; lang = flag + castellano at the rail; cut = IMAX or a word. Two rungs, one threshold (`SpecChip.rail` = 32): lockups at and above it, brand symbols (Dolby D, dts, the Blu-ray glyph, the flag) and the small badges below. A strip packs an axis at `.inkTight` and separates axes at `.inkGap`; a delta trails the axis's last mark.
