...
**The marks are real.** `Mark` (generated by `brandgen` into `Resources/Marks.xcassets`, provenance per file in `Resources/PROVENANCE.md`) holds the Dolby double-D and the Dolby Vision / Atmos / TrueHD / Digital Plus / Digital lockups, the dts symbol and wordmark, DTS-HD Master Audio, the HDR10 and HDR10+ badges, ULTRA HD, FLAC, Opus, Blu-ray Disc (lockup and glyph), Ultra HD Blu-ray, DVD, IMAX, the tabler 4K / 8K / HD / SD badges (MIT) and the Spanish civil flag - simple-icons (CC0) where it has them, Wikimedia Commons public-domain files everywhere else, all normalised to one ink at review, the flag alone keeping its colours. Trademarks, used nominatively: a chip says a copy carries the format the mark names. `Marks.swift` is the one binding from a value to its artwork; a value without an entry renders as words, exactly as before.

**Artwork first; a chip is one mark standing frameless.** The artwork IS the chip: a brand lockup keeps its shape, a brand symbol stands alone - no pill, no disc, no kind glyph around a mark. EVERY BOXED VALUE is one weight. 720p, HDR10 / HDR10+ / HLG, channels, Remux, WEB-DL / WEBRip / HDTV / CAM, DTS:X (its only vector is CC BY-NC-SA), TrueHD when the Atmos lockup already says Dolby, AAC / PCM / ALAC / MP3 / MP2 / Vorbis, castellano / latino / a code and every cut but IMAX are DRAWN badges in one geometry (box height = chip height, radius 0.25, stroke 0.08, side pad 0.26, word 0.62 semibold - the HDR10 badge's own proportion, box ≈ 1.5 × cap). SD / HD / 4K / 8K keep tabler's badge artwork, because 4K and 8K need real distinction and its letterforms give it: restroked to 1.12 on the 24-grid (1.12 over the 14-unit box = 0.08h) and rendered with the box at the chip height (`Mark.boxScale`), so tabler's badge and the drawn badge are one weight by construction. The Commons HDR10 / HDR10+ badges stay in the catalog as reference and bind nothing. Per axis: picture = resolution badge + range mark (SDR unsaid); sound = object mark, then the codec (lockup, or a word when an object lockup leads), then the channels; tier = disc lockup (Ultra HD Blu-ray when the strip knows 2160p) + Remux; lang = flag + castellano at the rail; cut = IMAX or a word. Two rungs, one threshold (`SpecChip.rail` = 32): lockups at and above it, brand symbols (Dolby D, dts, the Blu-ray glyph, the flag) and the small badges below. A strip packs an axis at `.inkTight` and separates axes at `.inkGap`; a delta trails the axis's last mark.

**Provenance sits on the mark**: `claim` is ghosted (0.55), `verified` full ink, `measured` full ink over a soft `inkRest` wash capsule, `delivered` the wash plus an `inkEdge` ring.

**Tone is the consumer's call**: `tone: .ink` (default) paints a mark in the axis's ink; `.brand` paints it its official hex - on the unwashed weights only, and never for a brand whose official colour is near-black (Dolby, HDR10, DVD: black on a dark chip is a missing logo, not a brand statement). The flag ignores tone by construction.

What it deliberately does NOT own: **ranking** - which of two copies is better is the daemon's ladder, config-as-data on its side; a chip renders one value honestly and knows nothing about order.
...
The raw values are the wire spellings, shared with the React `media-spec` item. `swift run mediaspec-example --check` prints every enum's vocabulary in declaration order and asserts it equals the pinned literal (the React item pins the same literal on its side, so a rename fails a gate instead of drawing a wrong chip), the label rules, the provenance order, the lenient-decode law, and the catalog against the reviewed manifest (every imageset is a `Mark`, every value's binding, the disc/inline aspect rule); without the flag it opens the sweep window - every resolution × range, every codec, the four weights, three heights, details, deltas, the two rungs in both tones, the artwork badges beside the drawn ones, and the whole mark catalog.
