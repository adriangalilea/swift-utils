...
func syntheticAspects(_ n: Int, seed: UInt64 = 9) -> [CGFloat] {
    ...
    let pool: [CGFloat] = [
        2.0 / 3.0, 16.0 / 9.0, 1.0, 4.0 / 3.0, 3.0 / 4.0, 2.39, 9.0 / 16.0, 3.4, 0.2,
    ]
...
func runChecks() -> Never {
    ...
    let rows = GalleryPack.justifiedRows(
        aspects: aspects, width: W, targetHeight: target, spacing: spacing)
    ...
    for (i, row) in rows.enumerated() {
        ...
        if row.height > target + 0.001 {
            fail("row \(i) height \(row.height) exceeds target \(target)")
        }
        if row.range.lowerBound != covered {
            fail("row \(i) starts at \(row.range.lowerBound), want \(covered) (gap/overlap)")
        }
    ...
    for n in [30, 77, 120, 199] {
        let head = GalleryPack.justifiedRows(
            aspects: Array(aspects[..<n]), width: W, targetHeight: target, spacing: spacing)
    ...
    if sel.anchor != 0 || sel.cursor != 0 {
        fail("selectAll must land anchor+cursor on the browse HEAD")
    }
    if sel.selectedInOrder(order) != order {
        fail("selectedInOrder must read back in browse order")
    }

    sel.select(7, .toggle, order: order)  // 7 leaves the selection
    if sel.selected.contains(7) { fail("toggle failed to remove") }
    if sel.openTarget(order: order) != 0 {
        fail("cursor outside selection: open must fall to the browse-order head")
    }
    sel.clear()
    if sel.openTarget(order: order) != nil {
        fail("empty selection must open nothing (never a ghost)")
    }
    ...
    if sel.selected != Set(0...4) {
        fail("shift-move must extend from the anchor: \(sel.selected)")
    }
